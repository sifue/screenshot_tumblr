...
function takeScreenshot() {
  chrome.tabs.captureVisibleTab(null, function(image) {
    var screenshotDataUrl = image;

    // GLOBAL PARAM SET
    chrome.tabs.executeScript(null, {code:"screenshotDataUrl = '" +  screenshotDataUrl + "';", runAt:'document_end'})
    // EXECUTE
    chrome.tabs.executeScript(null, {file:'post_and_share.js', runAt:'document_end'})
        ...
        var addSnapshotImageToTab = function(tabId, changedProps) {
            ...
            for (var i = 0; i < views.length; i++) {
                ...
                if (view.location.href == viewTabUrl) {
                    view.showScreenshot(screenshotDataUrl);
                    break;
                }
            }
        };
